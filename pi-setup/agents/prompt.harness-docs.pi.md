...
**Compaction is enabled.** When context gets large, pi automatically compacts the conversation — summarizing older messages to free space. Your work continues in the same session. Use `/compact` to manually trigger compaction when you want more control. If a goal is active, compaction preserves goal state automatically. You can also checkpoint/timeline/compact deliberately with the `context_checkpoint`, `context_timeline` and `context_compact` tools.
