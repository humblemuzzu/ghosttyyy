...
export default function tokenCompressor(pi: ExtensionAPI) {
  ...
  pi.on("tool_result", async (event, _ctx) => {
    ...
    // Support both `command` (pi default) and `cmd` (custom bash tool alias)
    ...
    // Strip custom bash tool's command echo prefix (e.g., "$ git status\n\n").
    // Without this, filters like git-status misdetect the output format because
    // "$ git status" is parsed as the first content line instead of "On branch main".
