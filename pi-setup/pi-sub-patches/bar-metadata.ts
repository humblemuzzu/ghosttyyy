...
const grokWindowVisible: ProviderMetadata["isWindowVisible"] = (_usage, window, settings, _model) => {
	if (!settings) return true;
	const ps = settings.providers.grok;
	if (window.label === "Week" || window.label === "Month" || window.label === "Usage") {
		return ps.windows.showWeek;
	}
	if (window.label === "Grok Build") return ps.windows.showBuild;
	if (window.label.startsWith("Extra")) return ps.windows.showExtra;
	return true;
};
...
export const PROVIDER_METADATA: Record<ProviderName, ProviderMetadata> = {
	...
	grok: {
		...BASE_METADATA.grok,
		isWindowVisible: grokWindowVisible,
	},
	zai: {
		...BASE_METADATA.zai,
