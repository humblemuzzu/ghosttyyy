...
const kimiWindowVisible: ProviderMetadata["isWindowVisible"] = (_usage, window, settings, _model) => {
	if (!settings) return true;
	const ps = settings.providers.kimi;
	if (window.label === "Week") return ps.windows.showWeek;
	if (window.label === "5h") return ps.windows.show5h;
	return true;
};

const crofWindowVisible: ProviderMetadata["isWindowVisible"] = (_usage, window, settings, _model) => {
	if (!settings) return true;
	const ps = settings.providers.crof;
...
export const PROVIDER_METADATA: Record<ProviderName, ProviderMetadata> = {
	...
	crof: {
		...BASE_METADATA.crof,
		isWindowVisible: crofWindowVisible,
	},
	kimi: {
		...BASE_METADATA.kimi,
		isWindowVisible: kimiWindowVisible,
	},
};
