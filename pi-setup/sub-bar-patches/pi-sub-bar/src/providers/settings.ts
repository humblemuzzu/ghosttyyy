...
export function buildProviderSettingsItems(settings: Settings, provider: ProviderName): SettingItem[] {
	...
	if (provider === "crof") {
		const crofSettings = ps as CrofProviderSettings;
		items.push(
			{
				id: "showDay",
				label: "Show Day Window",
				currentValue: crofSettings.windows.showDay ? "on" : "off",
				values: ["on", "off"],
				description: "Show the daily requests usage window.",
			},
			{
				id: "showCredits",
				label: "Show Credits",
				currentValue: crofSettings.windows.showCredits ? "on" : "off",
				values: ["on", "off"],
	...
	if (provider === "kimi") {
		const kimiSettings = ps as KimiProviderSettings;
		items.push(
			{
				id: "showWeek",
				label: "Show Week Window",
				...
				values: ["on", "off"],
				description: "Show the weekly quota window.",
			},
			{
				id: "show5h",
				label: "Show 5h Window",
				...
				values: ["on", "off"],
				description: "Show the 5-hour rate limit window.",
			},
		);
	}
...
export function applyProviderSettingsChange(
	...
	if (provider === "crof") {
		...
		switch (id) {
			case "showDay":
				crofSettings.windows.showDay = value === "on";
				break;
			case "showCredits":
				crofSettings.windows.showCredits = value === "on";
				break;
		}
	}
