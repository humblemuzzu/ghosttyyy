...
// list (flattened to depth 0) and are marked with a 📌 prefix. Ctrl+B toggles.
function getPinFilePath() {
    return join(getAgentDir(), "pinned-sessions.json");
}
function readPinnedPaths() {
    try {
        const raw = readFileSync(getPinFilePath(), "utf-8");
        ...
        for (const entry of parsed) {
            if (typeof entry !== "string")
                continue;
            set.add(canonicalizePath(entry) ?? entry);
        }
        return set;
    }
    catch {
...
function writePinnedPaths(paths) {
    const file = getPinFilePath();
    try {
        mkdirSync(dirname(file), { recursive: true });
        writeFileSync(file, JSON.stringify([...paths], null, 2));
    }
    catch {
        // best-effort; pinning is non-critical
    }
}
function isPinnedPath(pinnedSet, path) {
    return pinnedSet.has(canonicalizePath(path) ?? path);
}
function togglePinnedPath(path) {
    const pinned = readPinnedPaths();
    const key = canonicalizePath(path) ?? path;
    if (pinned.has(key)) {
        pinned.delete(key);
    }
    else {
    ...
    writePinnedPaths(pinned);
    return pinned;
}
/**
 * Reorder a flattened display list so pinned sessions appear first.
 * Pinned entries are flattened to depth 0 (no tree prefix) and tagged so the
 * renderer can show a marker. Relative order within each group is preserved.
 */
function applyPinning(list, pinnedSet) {
    if (pinnedSet.size === 0)
        return list;
    const pinned = [];
    const rest = [];
    for (const node of list) {
        if (isPinnedPath(pinnedSet, node.session.path)) {
            pinned.push({ ...node, depth: 0, isLast: true, ancestorContinues: [], pinned: true });
        }
        else {
            rest.push(node);
        }
...
class SessionSelectorHeader {
    ...
    render(width) {
        ...
        if (this.confirmingDeletePath !== null) {
            ...
            // LOCAL PATCH: pin hint
            hint2Parts.push(keyHint("app.session.pin", "pin"));
...
class SessionList {
    ...
    filterSessions(query) {
        ...
        // LOCAL PATCH: float pinned sessions to the top of the list.
        this.pinnedSet = readPinnedPaths();
        this.filteredSessions = applyPinning(this.filteredSessions, this.pinnedSet);
    ...
    render(width) {
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            // Build tree prefix (LOCAL PATCH: pinned sessions get a 📌 marker)
            const prefix = node.pinned ? "📌 " : this.buildTreePrefix(node);
    ...
    handleInput(keyData) {
        ...
        // LOCAL PATCH: pin/unpin selected session (Ctrl+B)
        if (kb.matches(keyData, "app.session.pin")) {
            const selected = this.filteredSessions[this.selectedIndex];
            if (selected) {
                togglePinnedPath(selected.session.path);
                this.filterSessions(this.searchInput.getValue());
                this.onTogglePin?.();
            }
            return;
...
export class SessionSelectorComponent extends Container {
    ...
    constructor(currentSessionsLoader, allSessionsLoader, onSelect, onCancel, onExit, requestRender, options, currentSessionFilePath) {
        ...
        // LOCAL PATCH: re-render after a pin toggle
        this.sessionList.onTogglePin = () => {
            this.requestRender();
        };
