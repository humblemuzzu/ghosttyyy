---
name: s-improve
description: "read-only codebase advisor (shadcn). audits a repo (bugs, security, perf, test coverage, tech debt, migrations, dx, docs, direction) and writes self-contained implementation plans in plans/ for another agent or you to execute — never edits source itself. use to audit a codebase or generate handoff plans. not for inline architecture answers — that's the oracle tool."
---
...
5. **If the user asks you to implement directly, decline and point at the plan** — offer to refine the plan, or hand a finished plan to a separate implementer (a plain `Task` subagent or another session) instead.
...
For repos of any real size, fan out with parallel read-only `Task` subagents — one per category (or cluster of related categories); use `finder` instead for search-only categories where locating code is the whole job. If you can't spawn subagents, audit directly yourself in category-priority order. **Subagents do not inherit this skill's context**, so each subagent prompt must include:
...
**Cost note (pi):** parallel `Task` subagents inherit the parent model, so `deep` (≤8 concurrent) on a large model is expensive. Prefer `standard` (the default, ≤4) unless the repo genuinely needs a full sweep; on a small repo, audit directly and skip the fan-out entirely.
...
- `--issues` (modifier on any planning invocation) → also publish each written plan as a GitHub issue via `gh`, URL recorded in the plan and index. Only with the explicit flag. Preflight: `gh auth status` succeeds and the repo has a GitHub remote — otherwise write the plan files as normal and say why issues were skipped. **Before creating any issue, check whether the repo is public (`gh repo view --json visibility`). If it is, warn the user that issues are publicly visible and get explicit confirmation before publishing any plan that describes a security vulnerability, credential location, or other sensitive finding.** Per plan: `gh issue create --title "<plan title>" --body-file <plan file>`; record each issue URL in the plan's Status block and the index.
